    ...
    if request.method == 'POST':
        ...
        if form.has_changed():
            if r['nocc_person_assigned'] != tour_data.nocc_person_assigned:
                ...
                tour_data.save()
                send_email(template='tour_assignment_nocc',
                           tour_data=tour_data)
                send_email(template='tour_assignment_visitor',
                           tour_data=tour_data)
                messages.success(
                    request, 'Tour details updated, emails sent to both requestor and NOCC representative with information that NOCC person is assigned to the tour')
            else:
...
def view_calendar(request):
    ...
    if request.method == 'POST':
        ...
        location_query = request.POST["location"]

    month_dates = calendar.Calendar().monthdatescalendar(year, month)
    tours = Tour.objects.filter(date__gte=datetime(year, month, 1)).exclude(date__gte=datetime(year, month, calendar.monthrange(
        year, month)[1])).exclude(status="Rejected").exclude(status="Canceled")
    if location_query != "All":
        tours = tours.filter(location__location=location_query)
    context = {
        'tours': tours.order_by('date', 'start_time'),
        'month': month,
        'year': year,
        'today': datetime.today().date(),
        'month_dates': month_dates,
        'year_range': range(2022, datetime.now().year+2),
...
def status_change(request, pk):
    ...
    if request.method == 'POST':
        ...
        if tour_data['nocc_person_assigned'] == None and tour_data['nocc_personnel_required'] == "Yes" and status == "Approved":
            messages.error(
                request, 'You can\'t approve this tour without NOCC person being assigned to it, please do that first')
            return redirect("/tour-details/"+pk)
        else:
            if tour_data['status'] != status:
                ...
                if status == "Approved":
                    ...
                    if status == "Rejected":
                        ...
                        send_email(template='cancellation',
                                   tour_data=tour_data2)
                        messages.warning(
                            request, 'Requestor will be informed via email that tour was canceled')
...
def new_tour(request):
    if request.method == 'POST':
        ...
        try:
            ...
            dbentry.tour_name = str(
                r['customer_or_group_name']) + "-" + str(r['date'])
            ...
            messages.success(
                request, 'Your tour has been submited and confirmation email sent to You. Please wait for approval from local representative.')
            return redirect("/thank-you")
        except:
            context = {
...
def get_avail_times(request):
    ...
    if request.method == 'GET' and r['location'] != '' and r['date'] != '':
        ...
        try:
            avail_start_time, avail_end_time = Availability.objects.filter(
                avail_date=f_date, location=f_location).values()[0]['avail_time'].split('-')
            avail_start_time = datetime.strptime(
            ...
            for tour in other_tours_that_day:
                ...
                while i < tour_end_time:
                    ...
                    if str_i in d_start_times:
                        print(str_i,' in start times')
                        d_start_times[str_i] = 0
                        print('d_start_times[i]',d_start_times[str_i])
                    #existing_tours_times.append(i.strftime("%H:%M"))
                    i = (datetime.combine(date.today(), i) +
                         timedelta(minutes=30)).time()
                    print('second i', i)
                    str_i = i.strftime("%H:%M")
                    if str_i in d_end_times:
                        print(str_i,' in end times')
                        d_end_times[str_i] = 0
            ...
            d_start_times = dict(sorted(d_start_times.items()))
            d_end_times = dict(sorted(d_end_times.items()))
            print(d_start_times)
            print(d_end_times)
...
def settings(request):
    ...
    if request.method == 'POST':
        ...
        if 'name' in r and 'email' in r:
            NoccRepresentatives.objects.create(
                location=location_instance, name=r['name'], email=r['email'])
            messages.success(
                request, f'{location_instance}\'s NOCC representatives list updated successfully')
        elif 'person_id' in r:
            person = NoccRepresentatives.objects.get(
                location__location=location_instance, id=int(r['person_id']))
            person.delete()
            messages.warning(
                request, f' Person removed from NOCC representatives list')
            ...
            messages.success(
                request, f'{location_instance}\'s time slots updated successfully')
...
def send_email_ics(pk):
    ...
    combined_date_time_start = datetime.combine(
        tour_data.date, tour_data.start_time)
    combined_date_time_end = datetime.combine(
        tour_data.date, tour_data.end_time)

    aware_combined_date_time_start = timezone.localize(
        combined_date_time_start)
    ...
    msg.attach_file(f'{directory}/{filename}', 'text/calendar')
    ...
    if template == 'new_request':
        ...
        for key, data in tour_data.__dict__.items():
            if key == '_state':
                ...
                if key == "status":
                    break
        to = [tour_data.requestor_email, tour_data.cc_this_request_to,
              tour_data.poc_email, 'rmirek@akamai.com']
        ...
        nocc_rep = NoccRepresentatives.objects.get(
            name=tour_data.nocc_person_assigned)
        ...
        to = ['rmirek@akamai.com', tour_data.requestor_email,
              tour_data.cc_this_request_to, tour_data.poc_email]
