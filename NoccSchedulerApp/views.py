...
def new_tour(request):
    ...
    form = TourForm()
    location_id = Location.objects.filter(location="Krakow").values()[0]['location']
    context={
        'time_slots': Availability.objects.filter(avail_date="2022-11-18", location=location_id).values()[0]['time_slots'].split(','),
        'form': form,
    }
    return render (request, "new-tour.html", context)