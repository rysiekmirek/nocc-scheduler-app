...
class TourForm(ModelForm):
    ...
    class Meta:
        model = Tour
        #fields = '__all__'
        exclude = ('id','status','nocc_person_assigned','feedback', 'tour_name', 'start_time', 'end_time', 'created', 'id', 'status' )
        widgets = {
            'date': DateTimeInput(attrs={'type': 'date', 'min': (date.today() + timedelta(days=1)), 'onchange': "this.form.submit()" }),
